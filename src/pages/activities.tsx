...
const parseLocation = (location: string): { label: string, link?: string, icon?: string, iconAlt?: string  } => {
...
const ActivityLocation = ({ location }: { location: string}) => {
    ...
    return (<span className={"activity--location"}>
        {loc.icon && <Image src={loc.icon} alt={loc.iconAlt!} width={iconSize} height={iconSize} />}
        {loc.link ? <Link href={loc.link}>{loc.label}</Link> : <span>{loc.label}</span>}
    </span>)
}
...
const ActivityEntry = ({ activity }: { activity: LearningActivity }) => {
    ...
    return (
        <Panel border className={"activity"}>
            ...
            <Detail className={"activity--contact"}>{activity.contactName}, {activity.contactRole}</Detail>
            <Ingress className={"activity--ingress"}>{activity.imageUrl && <img className={"activity--image"} src={activity.imageUrl!} alt={activity.contactName} />}{activity.description}</Ingress>
            <Detail className={"activity--locations"}>
                {activity.locations.map((loc, idx) => <ActivityLocation key={idx} location={loc} />)}
            </Detail>
        </Panel>
    )
}
